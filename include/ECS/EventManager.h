  ...
  public:
    template <typename Event> void connect(EventSubscriber<Event> *subscriber) {
        ...
        if (it == mSubscribers.end()) {
            ...
            it->second.push_back(
                std::shared_ptr<BaseEventSubscriber>(subscriber));
        }
    }
    ...
    template <typename Event> void event(const Event *event) {
        ...
        if (it != mSubscribers.end()) {
            ...
            for (auto &base : subscribers) {
                auto sub =
                    reinterpret_cast<EventSubscriber<Event> *>(base.get());

                const auto boundFunc = std::bind(
                    &EventSubscriber<Event>::receive, sub, this, *event);
    ...
    EventManager(Allocator alloc)
        : mEntAlloc(alloc), mEntities({}, EntityPtrAllocator(alloc)),
          mSubscribers({}, 0, std::hash<TypeIndex>(),
